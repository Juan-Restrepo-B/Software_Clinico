<?php
    ...
    CONCAT(COALESCE(d.nodoc, ' '), ' ', COALESCE(d.apdoc, ' '), 
    COALESCE(n.nomnur, ' '), ' ', COALESCE(n.apenur, ' '),
    COALESCE(p.nompa, ' '), ' ', COALESCE(p.apepa, ' ')) AS Nombre
    FROM users u 
...
<head>
    ...
    <title><?php echo htmlspecialchars($setting['nomem']); ?>| Panel administrativo</title>
</head>
<body>
    ...
    <section id="content">
        ...
        <!-- MAIN -->
        <main>
            <h1 class="title">Bienvenido <?php echo mb_convert_case(mb_strtolower($users, "UTF-8"), MB_CASE_TITLE, "UTF-8"); ?></h1>
            ...
            <div class="info-data">
                ...
                <div class="card">
                    <div class="head">
                        <div>
                            <?php 
                                            $sql = "SELECT SUM(monto) AS total FROM events WHERE DATE(fere) = CURDATE()";
                                            $result = $connect->query($sql); //$pdo sería el objeto conexión
                                            $total = $result->fetchColumn();
                    ...
                    <div class="head">
                        <div>
                            ...
                            if (isset($connect)) {
                                ...
                                try {
                                    $sql = "SELECT SUM(monto) AS total FROM events WHERE YEAR(fere) = YEAR(CURDATE()) AND MONTH(fere) = MONTH(CURDATE())";
                                    $result = $connect->query($sql);

                                    if ($result) {
                                        $total = $result->fetchColumn();
                                        $total = $total ?: '0.00';  // Utiliza operador ternario para manejar NULL
                                    } else {
                                        $total = '0.00';
            ...
            <tbody>
                <?php foreach($data as $d):?>
                    <tr>
                        ...
                        <td data-title="Paciente"><?php echo mb_strtoupper($d->nodoc, 'UTF-8') ?>&nbsp;<?php echo mb_strtoupper($d->apdoc, 'UTF-8') ?></td>
                        <td data-title="Especialidad"><?php echo mb_convert_case(mb_strtolower($d->nomesp), MB_CASE_TITLE, "UTF-8") ?></td>
            ...
            <tbody>
                <?php foreach($data as $d):?>
                    <tr>
                        ...
                        <td data-title="Paciente"><?php echo mb_strtoupper($d->nompa, 'UTF-8') ?>&nbsp;<?php echo mb_strtoupper($d->apepa, 'UTF-8') ?></td>
